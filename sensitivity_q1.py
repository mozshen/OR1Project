import pulp
#%%
#function with rhs input
def rhs_sensitivity(rhs= 27000, coef= 2):
    ...
    solver = pulp.getSolver(solver= 'PULP_CBC_CMD', msg=1, timeLimit=30, gapRel=0, gapAbs=0)
...
for demand_27 in range(10000, 51000, 1000):
    opt_values.append(rhs_sensitivity(rhs= demand_27))
rhs_values_1= opt_values.copy()

%matplotlib qt

x= list(range(10000, 51000, 1000))
plt.figure(dpi= 200)
plt.scatter(x, rhs_values_1, linewidths= 0.1)
plt.plot(x, rhs_values_1)
plt.xlabel('Last Period Demand')
plt.ylabel('Total Cost (Million Euros)')
plt.title('Effect of Changing Last Period Demand in Part A')
...
%matplotlib qt
coef_values_1= opt_values.copy()

x= np.linspace(0.1, 5, 50)
plt.figure(dpi= 200)
plt.scatter(x, coef_values_1, linewidths= 0.1)
plt.plot(x, opt_values)
plt.xlabel('Cost for Extra Production in the First Type Plant')
plt.ylabel('Total Cost (Million Euros)')
plt.title('Effect of Changing the Cost for Extra Production in the First Type Plant in Part A')
plt.axvline(2, c= 'g')
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
